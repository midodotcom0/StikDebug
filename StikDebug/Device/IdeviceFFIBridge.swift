...
///
/// This used to build its own `tunnel_create_rppairing` session against port 49152,
/// which hard-wired location simulation to the Wi-Fi-only transport even when another
/// one was available. It now runs on the shared session from `DeviceTransport`, so it
/// inherits the CoreDeviceProxy fallback and survives a Wi-Fi to cellular handoff.
///
/// `deviceIP` and `pairingFile` are accepted for source compatibility with existing
/// call sites; the transport layer resolves both from settings.
func simulate_location(_ deviceIP: String, _ latitude: Double, _ longitude: Double, _ pairingFile: String) -> Int32 {
    do {
        return try DeviceTransport.shared.withLocationChannel { channel in
            if let ffiError = location_simulation_set(channel, latitude, longitude) {
                throw TransportError.ffi(ffiError, fallback: "Failed to set simulated location")
            }
            return LocationSimulationStatus.ok
        }
    } catch let error as NSError {
        LogManager.shared.addErrorLog("Location simulation failed: \(error.localizedDescription)")
        switch error.code {
        case -18:
        ...
        default:
            return DeviceTransport.shared.isConnected
...
func clear_simulated_location() -> Int32 {
    // Nothing was ever set on this session, so there is nothing to clear and no
    // reason to build a connection just to say so.
    guard DeviceTransport.shared.hasLocationChannel else {
        return LocationSimulationStatus.locationClear
    }

    do {
        return try DeviceTransport.shared.withLocationChannel { channel in
            if let ffiError = location_simulation_clear(channel) {
                throw TransportError.ffi(ffiError, fallback: "Failed to clear simulated location")
            }
            return LocationSimulationStatus.ok
        }
    } catch let error as NSError {
        LogManager.shared.addErrorLog("Clearing simulated location failed: \(error.localizedDescription)")
        return LocationSimulationStatus.locationClear
    }
}
